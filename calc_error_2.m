function error_struct = calc_error_2(XData, PData, Xs, numSamples, N)
    ...
    error_struct.msePosErr = mseError(:, stateMasks.pos);
    error_struct.mseVelErr = mseError(:, stateMasks.vel);
    error_struct.mseAccBiasErr = mseError(:, stateMasks.acc_bias);
    error_struct.mseMagBiasErr = mseError(:, stateMasks.mag_bias);
    error_struct.mseThetaErr = mseError(:, stateMasks.theta);

    error_struct.stdPosErr = stdError(:, stateMasks.pos);
    error_struct.stdVelErr = stdError(:, stateMasks.vel);
    error_struct.stdAccBiasErr = stdError(:, stateMasks.acc_bias);
    error_struct.stdMagBiasErr = stdError(:, stateMasks.mag_bias);
    error_struct.stdThetaErr = stdError(:, stateMasks.theta);
