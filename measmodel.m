classdef measmodel
    ...
    methods (Static)
        function obj = cvmeasmodel(sigma)
            ...
            %       obj.h: function handle return a measurement --- 2 x 1
            %       matrix 
            obj.d = 2;
        ...
        function obj = cvmeasmodel_1D(sigma)
            %CVMEASMODEL creates the measurement model for a 1D nearly
            %constant velocity motion model 
            %INPUT: sigma: standard deviation of measurement noise ---
            %       scalar 
            %OUTPUT:obj.d: measurement dimension --- scalar
        ...
        function obj = bearingmeasmodel(sigma, s)
            ...
            %       scalar 
            %       obj.H: function handle to call measurement model
            %       Jacobian --- 1 x (state dimension) vector 
            %       obj.R: measurement noise covariance --- scalar
            %NOTE:  the measurement model assumes that in the state vector
            %       the first two entries are the X-position and
        ...
        function obj = dualbearingmeasmodel(sigma, s1, s2)
            ...
            %OUTPUT:obj.d: measurement dimension --- scalar
            %       obj.h: function handle to generate measurement --- 2 x
            %       1 vector  
            %       obj.H: function handle to call measurement model
            %       Jacobian --- 2 x (state dimension) vector 
            %       obj.R: measurement noise covariance --- 2 x 2 matrix
            %NOTE:  the measurement model assumes that in the state vector
            %       the first two entries are the X-position and
            %       Y-position, respectively.  
        ...
        function obj = rangebearingmeasmodel(sigma_r, sigma_b, s)
            %RANGEBEARINGMEASUREMENT creats the range/bearing measurement
            %model 
